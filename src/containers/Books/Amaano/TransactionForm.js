...
const TransactionForm = (props) => {
  ...
  const deleteTransaction = async () => {
    await deleteFunction(
      ...
      `${constants.baseUrl}/depositor-transactions/${props.tran?._id}`,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) =>  {
        ...
        values.depositor = props.instance?._id
        values.user = props.instance?.user?._id
      setDisabled(true)
      console.log(props.update)

      if (props.update){
        const res = await axios.patch(`${constants.baseUrl}/depositor-transactions/${props.tran?._id}`, values,
        {
          headers: {
            "authorization": token
          }
        ...
        const res = await axios.post(`${constants.baseUrl}/depositor-transactions`, values,
        {
          headers: {
            "authorization": token
          }
